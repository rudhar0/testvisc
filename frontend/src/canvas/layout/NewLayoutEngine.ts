// frontend/src/canvas/layout/NewLayoutEngine.ts
      ...
      if (type === 'function') {
        if (element === 'function_call') {
          ...
          if (!this.elements.has(frameId)) {
            ...
            const newFrame: LayoutElement = {
              ...
              type: 'function',
              subtype: element,
              x: parentFrame ? parentFrame.x + PADDING : STACK_FRAME_X,
              y: parentFrame ? (parentFrame.y + parentFrame.height + PADDING) : STACK_FRAME_Y,
              width: STACK_FRAME_WIDTH - (parentFrame ? PADDING * 2 : 0),
              height: 100, // Initial height
              data: payload,
              children: [],
        ...
        if (parentFrame) {
            ...
            if (!this.elements.has(id)) {
                const x = parentFrame.x + PADDING + (parentFrame.children?.length || 0) * (VAR_WIDTH + PADDING);
                const y = parentFrame.y + PADDING + 40;
