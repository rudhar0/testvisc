        ...
        if (state?.callStack && state.callStack.length > 0) {
          ...
          if (frame.locals) {
            ...
            variables.forEach((variable: any, index: number) => {
              // If a variable explicitly has a birthStep later than this step, skip for now
              ...
              // Only create the variable element at declaration if it has an initial value
              // or if the backend explicitly annotated a birthStep for it. Otherwise,
              // defer creation until an assignment step creates/initializes it so variables
              // don't all appear at the declaration step.
              ...
              if (!hasInitialValue && !wasAnnotated) {
                // Defer creation; assignment handler will create the variable when it gets a value
              ...
              if (!this.elementHistory.has(varId)) {
                ...
                // If the variable includes a birthStep, use it as the element's creation step
